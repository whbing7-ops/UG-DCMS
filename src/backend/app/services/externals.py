...
def add_hardware_compatibility(conn: psycopg.Connection, version_id: str,
    ...
    if not hardware_version.strip(): raise ValueError("硬件版本不能为空")
    ...
    if hw is None: raise LookupError("所选硬件件号不存在")
    if hw["lifecycle_status"] == "OBSOLETE": raise ValueError("已作废硬件不能新增适装关系")
...
def add_version_package(conn: psycopg.Connection, software_number: str, *, version: str,
    ...
    if so is None: raise LookupError(f"软件对象不存在: {software_number}")
...
def submit_version(conn: psycopg.Connection, version_id: str,
    ...
    if not sv["hash_sha256"] or not sv["package_storage_key"]:
        raise ValueError("提交前必须上传软件压缩包，由系统自动登记 SHA-256")

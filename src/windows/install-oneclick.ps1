...
$releaseName = 'app-1.0.0-rc2.34-' + (Get-Date -Format 'yyyyMMddHHmmss')
...
$runtimeName = 'venv-1.0.0-rc2.34-' + (Get-Date -Format 'yyyyMMddHHmmss')
...
if([int]$migrationCount -ne 20){ Fail "数据库迁移数量异常：期望 20，实际 $migrationCount" }
Write-Step '数据库迁移完整性检查通过：20/20（综合演示业务数据已清理，仅保留账户与基础配置）'

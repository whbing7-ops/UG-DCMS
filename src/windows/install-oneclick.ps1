...
$releaseName = 'app-1.0.0-rc2.37-' + (Get-Date -Format 'yyyyMMddHHmmss')
...
$runtimeName = 'venv-1.0.0-rc2.37-' + (Get-Date -Format 'yyyyMMddHHmmss')
...
if([int]$migrationCount -ne 21){ Fail "数据库迁移数量异常：期望 21，实际 $migrationCount" }
Write-Step '数据库迁移完整性检查通过：21/21（综合演示业务数据已清理，仅保留账户与基础配置）'

...
$releaseName = 'app-1.0.0-rc2.44-' + (Get-Date -Format 'yyyyMMddHHmmss')
...
$runtimeName = 'venv-1.0.0-rc2.44-' + (Get-Date -Format 'yyyyMMddHHmmss')
...
if([int]$migrationCount -ne 25){ Fail "数据库迁移数量异常：期望 25，实际 $migrationCount" }
Write-Step '数据库迁移完整性检查通过：25/25（申请人审批操作已就绪，升级保留已有业务数据）'

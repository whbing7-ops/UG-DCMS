...
$releaseName = 'app-1.0.0-rc2.42-' + (Get-Date -Format 'yyyyMMddHHmmss')
...
$runtimeName = 'venv-1.0.0-rc2.42-' + (Get-Date -Format 'yyyyMMddHHmmss')
...
if([int]$migrationCount -ne 24){ Fail "数据库迁移数量异常：期望 24，实际 $migrationCount" }
Write-Step '数据库迁移完整性检查通过：24/24（申请人审批操作已就绪，升级保留已有业务数据）'

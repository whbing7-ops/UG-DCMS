...
function workQueue(data){
  const items=[...data.inbox.map(r=>({title:r.title,number:r.request_number,state:'待我审批',url:'#/approval/'+r.id})),
...
function notificationSettings(d){
  ...
  return panel('Windows 系统通知',el('div',{},connection,
    ...
    el('div',{class:'actions'},link('下载 Windows 通知助手','https://github.com/whbing7-ops/UG-DCMS/releases/download/rc2.41/UG-DCMS-Notify-rc2.41.exe','btn'),

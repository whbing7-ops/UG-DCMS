...
function Validate-InstallerSource {
  ...
  if($provisionText -notmatch [regex]::Escape("app-$expected-")){ throw 'Versioned release naming is missing.' }
  ...
  if($provisionText -notmatch [regex]::Escape("venv-$expected-")){ throw 'Versioned runtime naming is missing.' }
  ...
  if($provisionText -notmatch '数据库迁移完整性检查通过：21/21'){ throw 'Database migration completeness check is missing.' }
  ...
  if($migrations.Count -ne 21){ throw "Expected 21 DB migrations, found $($migrations.Count)." }
...
try {
  ...
  Write-Host "Setup.exe created: installer\output\UG-DCMS-Setup-1.0.0-rc2.35.exe" -ForegroundColor Green

...
function Validate-InstallerSource {
  ...
  if($provisionText -notmatch '数据库迁移完整性检查通过：22/22'){ throw 'Database migration completeness check is missing.' }
...
try {
  ...
  Write-Host "Setup.exe created: installer\output\UG-DCMS-Setup-1.0.0-rc2.38.exe" -ForegroundColor Green

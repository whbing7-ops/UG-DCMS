...
function Validate-InstallerSource {
  ...
  if($provisionText -notmatch '数据库迁移完整性检查通过：27/27'){ throw 'Database migration completeness check is missing.' }
  ...
  if($migrations.Count -ne 27){ throw "Expected 27 DB migrations, found $($migrations.Count)." }
...
try {
  ...
  Write-Host "Setup.exe created: installer\output\UG-DCMS-Setup-1.0.0-rc2.45.exe" -ForegroundColor Green

...
function Validate-InstallerSource {
  ...
  foreach($required in @($issPath,$cmdPath,$provisionPath,(Join-Path $root 'windows\migrate-native.ps1'),(Join-Path $root 'backend\requirements.txt'))){
  ...
  if($provisionText -notmatch 'app-1\.0\.0-rc2\.12-'){ throw 'Versioned release naming is missing.' }
  ...
  if($provisionText -notmatch 'venv-1\.0\.0-rc2\.12-'){ throw 'Versioned runtime naming is missing.' }
...
try {
  ...
  Write-Host "Setup.exe created: installer\output\UG-DCMS-Setup-1.0.0-rc2.12.exe" -ForegroundColor Green

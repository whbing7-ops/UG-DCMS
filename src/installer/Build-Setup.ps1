...
function Validate-InstallerSource {
  ...
  if($provisionText -notmatch '数据库迁移完整性检查通过：26/26'){ throw 'Database migration completeness check is missing.' }
  ...
  if($migrations.Count -ne 26){ throw "Expected 26 DB migrations, found $($migrations.Count)." }

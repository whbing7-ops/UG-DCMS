...
function Validate-InstallerSource {
  ...
  if($provisionText -notmatch 'app-1\.0\.0-rc2\.17-'){ throw 'Versioned release naming is missing.' }
  ...
  if($provisionText -notmatch 'venv-1\.0\.0-rc2\.17-'){ throw 'Versioned runtime naming is missing.' }
...
try {
  ...
  Write-Host "Setup.exe created: installer\output\UG-DCMS-Setup-1.0.0-rc2.17.exe" -ForegroundColor Green

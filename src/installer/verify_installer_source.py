...
ok(len(migs) == 24, f'expected 24 migrations, found {len(migs)}')
...
checks = {
    'builder migration count': 'Count -ne 24' in build and '24/24' in build,
    'CMD output version': 'rc2.43.exe' in (ROOT/'installer'/'BUILD-SETUP.cmd').read_text(),
    ...
    'versioned runtime name': 'venv-1.0.0-rc2.43-' in ps,
    'versioned release name': 'app-1.0.0-rc2.43-' in ps,
    ...
    'migration count gate': '数据库迁移完整性检查通过：24/24' in ps,
    ...
    'frontend upgrade invalidates cache': 'FreshStaticFiles' in main_api
        and 'no-store, max-age=0' in main_api and '?v=rc2.43' in index_html,
...
# Ordering invariants
try:
    mig_done = ps.index("Write-Step '数据库迁移完整性检查通过：24/24（申请人审批操作已就绪，升级保留已有业务数据）'")
...
print(f' [PASS] migrations UTF-8/order: {len(migs)}/24')

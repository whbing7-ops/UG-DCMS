...
ok(len(migs) == 26, f'expected 26 migrations, found {len(migs)}')
...
checks = {
    'builder migration count': 'Count -ne 26' in build and '26/26' in build,
    ...
    'migration count gate': '数据库迁移完整性检查通过：26/26' in ps,
...
# Ordering invariants
try:
    mig_done = ps.index("Write-Step '数据库迁移完整性检查通过：26/26（申请人审批操作已就绪，升级保留已有业务数据）'")
...
print(f' [PASS] migrations UTF-8/order: {len(migs)}/26')

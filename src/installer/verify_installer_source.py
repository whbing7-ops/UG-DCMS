...
ok(len(migs) == 21, f'expected 21 migrations, found {len(migs)}')
...
checks = {
    ...
    'CMD output version': 'rc2.37.exe' in (ROOT/'installer'/'BUILD-SETUP.cmd').read_text(),
    ...
    'versioned runtime name': 'venv-1.0.0-rc2.37-' in ps,
    'versioned release name': 'app-1.0.0-rc2.37-' in ps,
    ...
    'migration count gate': '数据库迁移完整性检查通过：21/21' in ps,
    ...
    'frontend upgrade invalidates cache': 'FreshStaticFiles' in main_api
        and 'no-store, max-age=0' in main_api and '?v=rc2.37' in index_html,
...
# Ordering invariants
try:
    mig_done = ps.index("Write-Step '数据库迁移完整性检查通过：21/21（综合演示业务数据已清理，仅保留账户与基础配置）'")
...
print(f' [PASS] migrations UTF-8/order: {len(migs)}/21')

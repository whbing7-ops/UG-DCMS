...
ok(len(migs) == 17, f'expected 17 migrations, found {len(migs)}')
...
checks = {
    ...
    'versioned runtime name': 'venv-1.0.0-rc2.28-' in ps,
    'versioned release name': 'app-1.0.0-rc2.28-' in ps,
    ...
    'migration count gate': '数据库迁移完整性检查通过：17/17' in ps,
    ...
    'frontend upgrade invalidates cache': 'FreshStaticFiles' in main_api
        and 'no-store, max-age=0' in main_api and '?v=rc2.28' in index_html,
...
# Ordering invariants
try:
    mig_done = ps.index("Write-Step '数据库迁移完整性检查通过：17/17（综合演示业务数据已清理，仅保留账户与基础配置）'")
...
print(f' [PASS] migrations UTF-8/order: {len(migs)}/17')

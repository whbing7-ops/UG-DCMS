...
with sync_playwright() as pw:
    ...
    try:
        ...
        audit_row=admin.get_by_role('row').filter(has=admin.get_by_role('cell',name='USER_CREATE',exact=True)).filter(has=admin.get_by_role('cell',name=username,exact=True))

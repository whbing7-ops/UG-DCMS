...
'@
  ...
  $args=@('--headless=new','--disable-gpu','--no-first-run','--no-default-browser-check','--dump-dom','http://127.0.0.1:8765/__ugdcms_dom_smoke.html')
  ...
  if($p.ExitCode -ne 0){
    ...
    throw ('Edge DOM smoke exited '+$p.ExitCode)
  }
  $dom=Get-Content $outFile -Raw -Encoding UTF8
  if(-not $dom.Contains('data-ugdcms-dom-smoke="pass"')){
    Write-Host '--- dumped DOM ---'
    Write-Host $dom
    throw 'real browser DOM smoke failed'
  }
  if($dom.Contains('[object HTMLHeadingElement]') -or $dom.Contains('[object HTMLAnchorElement]')){
    throw 'browser DOM still contains stringified HTML elements'
  }
  Write-Host 'FRONTEND EDGE DOM SMOKE PASS: nested sidebar-style arrays render into real headings and links.'
}
finally {
